<?php
    ...
    if(isset($_POST['btn1'])){
        $cm = $_POST["cm"];
        if($cm>0){
            $yd = '<div class="alert alert-danger">'.$cm.' (cm) เป็น '.($cm * 0.010936133).' (yd)</div>';
        }else{
            $yd = '<div class="alert alert-danger">กรุณาป้อนข้อมูล</div>';
        }
    }
    if(isset($_POST['btn2'])){
        header('Location:yd&cm.php');
    }
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>เซนติเมตร เป็น หลา</title>
</head>
<body>
    <?php require('./nav.php') ?>
    <div class="container text-center">
            <div class="row">
                ...
                <div class="col-6 border rounded mt-2 p-2 ">
                    <form method="post">
                    <h3>เซนติเมตร(cm) เป็น หลา(yd)</h3>
