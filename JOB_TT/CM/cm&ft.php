<?php
    ...
    if(isset($_POST['btn1'])){
        $cm = $_POST["cm"];
        if($cm>0){
            $ft = '<div class="alert alert-danger">'.$cm.' (cm) เป็น '.($cm * 0.032808399).' (ft)</div>';
        }else{
            $ft = '<div class="alert alert-danger">กรุณาป้อนข้อมูล</div>';
        }
    }
    if(isset($_POST['btn2'])){
        header('Location:ft&cm.php');
    }
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>เซนติเมตร เป็น ฟุต</title>
</head>
<body>
    <?php require('./nav.php') ?>
    <div class="container text-center">
            <div class="row">
                ...
                <div class="col-6 border rounded mt-2 p-2 ">
                    <form method="post">
                    <h3>เซนติเมตร(cm) เป็น ฟุต(ft)</h3>
